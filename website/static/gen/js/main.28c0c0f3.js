...
if(selected=="tech_img1"){caption_elem.innerText="This is about Python"}else if(selected=="tech_img2"){caption_elem.innerText="This is about HTML5"}else if(selected=="tech_img3"){caption_elem.innerText=`Oh CSS.Definitely one of the more tricky languages to work with,that is,until SaaS really picked up in popularity.I generally use SaaS because it saves a lot of rule-writing,what with the use of mixins`}else if(selected=="tech_img4"){caption_elem.innerText="This is about JS5. I'm in the middle of learning some of the changes in es6.  "}else if(selected=="tech_img5"){caption_elem.innerText="This is about MYSQL"}else if(selected=="tech_img6"){caption_elem.innerText="This is about Photoshop"}else if(selected=="tech_img7"){caption_elem.innerText="This is about Adobe XD."}else if(selected=="tech_img8"){caption_elem.innerText="This is about github."}else if(selected=="tech_img8"){caption_elem.innerText="This is about windows."}
}
...
function alter_image(scrollPos){let filter_max=0
if(scrollPos>2500){if(filter_min>filter_max){img.style.filter="grayscale("+filter_min+"%)"
filter_min-=2;}}}
function dissolve_divider(scrollPos){let divider=document.querySelector('.shimmering_div')
if(scrollPos>10){divider.style.display="none"}}
document.addEventListener('scroll',function(e){lastKnownScrollPosition=window.scrollY;if(!ticking){window.requestAnimationFrame(function(){shift_digital(lastKnownScrollPosition);alter_image(lastKnownScrollPosition);dissolve_divider(lastKnownScrollPosition);ticking=false;});ticking=true;}});var world_ctnr=document.querySelector('#world_ctnr')
...
var star_cluster_geometry=new THREE.SphereBufferGeometry(3,20,15);var star_cluster_material=new THREE.PointsMaterial({size:0.02,transparent:true,color:0xa165ff})
