...
if(tech_ctnr){imgs=tech_ctnr.querySelectorAll('.tech_img_ctnr')
for(let img of imgs){img.addEventListener('click',alter_tech_interface,false)
img.addEventListener('mouseover',alter_tech_interface,false)}}
let selected="tech_img1";let selected_elem=document.querySelector(`#${selected}`)
if(selected_elem){selected_elem.click()}
function alter_tech_interface(){unselected=tech_ctnr.querySelector(`#${selected}`)
...
filter_min-=3;}}else if(scrollPos<2000){img.style.filter="grayscale(100%)"
filter_min=100}}
let divider=document.querySelector('.shimmering_div')
function dissolve_divider(scrollPos){if(scrollPos>10){divider.classList.add("dissolving_div")
setTimeout(function(){divider.style.display="none"},400)}}
function check_no_scroll(scrollPos){setTimeout(function(){divider.classList.add("enlarge_div")
...
divider.appendChild(scroll_alert)},30000/3)}
if(divider){check_no_scroll()}
document.addEventListener('scroll',function(e){lastKnownScrollPosition=window.scrollY;if(!ticking){window.requestAnimationFrame(function(){shift_digital(lastKnownScrollPosition);dissolve_divider(lastKnownScrollPosition);if(screen.width>=1600){alter_image(lastKnownScrollPosition);alert('right')}
ticking=false;});ticking=true;}});var world_ctnr=document.querySelector('#world_ctnr')
