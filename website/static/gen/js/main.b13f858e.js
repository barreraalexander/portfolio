...
var light3=new THREE.PointLight(0xa165ff,1,1000)
light3.position.set(-100,10,10);scene.add(light3)
...
const particle_material=new THREE.PointsMaterial({size:0.005,transparent:true,color:'white',blur:.5})
const particles_mesh=new THREE.Points(particles_geomtery,particle_material)
scene.add(particles_mesh)
var star_cluster_geometry=new THREE.SphereBufferGeometry(1.5,20,15);var star_cluster_material=new THREE.PointsMaterial({size:0.02,transparent:true,color:'white'})
var start_cluster_mesh=new THREE.Points(star_cluster_geometry,star_cluster_material)
start_cluster_mesh.position.x=-2.5;start_cluster_mesh.position.y=-1.5;start_cluster_mesh.position.z=-2;scene.add(start_cluster_mesh)
...
mesh1.rotation.y+=0.01;mesh2.rotation.x+=0.01;start_cluster_mesh.rotation.x+=0.0001;particles_mesh.rotation.y=(.0009*elasped_time)
